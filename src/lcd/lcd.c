...
void LCD_Show_tgdr(u16 x0,u16 y0,u8* img) 
{
  int w = 20, h = 20;
  for(int i = 0; i < w; ++i) {
	int zero_padding = 0;
	while(zero_padding < h && img[(i*h + zero_padding)<<1] == 0 && img[(i*h + zero_padding)<<1|1] == 0)  ++zero_padding;
	for(int j = zero_padding; j < h - zero_padding; ++j) {
		LCD_Address_Set(x0 + j, y0 + i, x0 + j, y0 + i);
		LCD_WR_DATA8(img[(i*h + j)<<1]);
		LCD_WR_DATA8(img[(i*h + j)<<1|1]);
	}
	for(int j = h - zero_padding; j < h; ++j) {
		LCD_Address_Set(x0 + j, y0 + i, x0 + j, y0 + i);
